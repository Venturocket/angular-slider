angular.module('vr.directives.slider',[])
	.directive('slider', ['$timeout', '$document', '$interpolate', function($timeout, $document, $interpolate) {
		var angularize, bindHtml, gap, halfWidth, hide, offset, offsetLeft, pixelize, roundStep, stepBuffer, show, width, STRETCH_RESISTANCE;
		...
		offsetLeft = function(element) {
			...
			return rawDom.getBoundingClientRect().left + scrollX;
		};
		...
		return {
			...
			template: "<span class='bar full'></span><span class='bar selection'></span><span class='bar unselected low'></span><span class='bar unselected high'></span><span class='pointer low'></span><span class='pointer high'></span><span class='bubble low'></span><span class='bubble high'></span><span class='bubble middle'></span><span class='bubble selection'></span><span class='bubble limit floor'></span><span class='bubble limit ceiling'></span>",
			compile : function(element, attributes) {
				var ceilBub, cmbBub, e, flrBub, fullBar, highBub, lowBub, maxPtr, minPtr, range, refHigh, refLow, refBar, refSel, refUnSelLow, refUnSelHigh, selBar, unSelBarLow, unSelBarHigh, selBub, watchables, _i, _len, _ref, _ref1, startSymbol, endSymbol;
				...
				_ref = (function() {
					...
					return _results;
				})(), fullBar = _ref[0], selBar = _ref[1], unSelBarLow = _ref[2], unSelBarHigh = _ref[3], minPtr = _ref[4], maxPtr = _ref[5], lowBub
					= _ref[6], highBub = _ref[7], cmbBub = _ref[8], selBub = _ref[9], flrBub = _ref[10], ceilBub = _ref[11];
				refLow = range ? 'ngModelLow' : 'ngModel';
				refHigh = 'ngModelHigh';
				refSel = 'selectBar';
				startSymbol = $interpolate.startSymbol();
				endSymbol = $interpolate.endSymbol();
				bindHtml(ceilBub, startSymbol+" translation(ceiling) "+endSymbol);
				bindHtml(flrBub, startSymbol+" translation(floor) "+endSymbol);
				bindHtml(selBub, "Range: "+startSymbol+" translation(diff) "+endSymbol);
				bindHtml(lowBub, startSymbol+" translation(" + refLow + ")"+endSymbol);
				bindHtml(highBub, startSymbol+" translation(" + refHigh + ") "+endSymbol);
				bindHtml(cmbBub, startSymbol+" translation(" + refLow + ") "+endSymbol+" - "+startSymbol+" translation(" + refHigh + ") "+endSymbol);
				if(!range) {
				...
				return {
					post: function(scope, element, attributes) {
						...
						updateDOM = function() {
							var adjustBubbles, fitToBar, percentOffset, valueOffset, percentToOffset, percentValue, setBindings, setPointers, percentStretch, onEnd, onMove, onStart, onClick, dragRange, lowOffset, highOffset, newLow, newHigh, pointer, ref, startCoords;
							...
							fitToBar = function(element) {
								return offset(element, percentToOffset(percentOffset(Math.min(Math.max(minOffset, offsetLeft(element)), maxOffset))));
							};
							percentStretch = function(percent, maxPercent, end) {
								...
								var stretch = scope.stretch ? parseInt(scope.stretch) : STRETCH_RESISTANCE;
								return sign * Math.pow(Math.min(Math.abs(percent / maxPercent * 2), 1), stretch) * maxPercent / 2;
							};
							setPointers = function() {
								var newHighValue, newLowValue, newLowPercent, newHighPercent, maxPercent, bufferPercent, selBarPercent, ptrWidthOffset;
								...
								offset(lowBub, percentToOffset(percentOffset(offsetLeft(minPtr) - halfWidth(lowBub) + pointerHalfWidth)));
								if(range) {
									...
									offset(highBub, percentToOffset(percentOffset(offsetLeft(maxPtr) - halfWidth(highBub) + pointerHalfWidth)));
									ptrWidthOffset = percentOffset(pointerHalfWidth + minOffset);
									selBarPercent = newLowPercent + ptrWidthOffset;
									offset(selBar, percentToOffset(selBarPercent));
									selBar.css({
										width: percentToOffset(newHighPercent - newLowPercent)
									});
									unSelBarLow.css({
										left : 0,
										width: percentToOffset(newLowPercent + ptrWidthOffset)
									});
									offset(unSelBarHigh, percentToOffset(newHighPercent + ptrWidthOffset));
									unSelBarHigh.css({
										right: 0
									});
									offset(selBub, percentToOffset(selBarPercent + percentOffset(halfWidth(selBar) - halfWidth(selBub) + minOffset)));
									return offset(cmbBub, percentToOffset(selBarPercent + percentOffset(halfWidth(selBar) - halfWidth(cmbBub) + minOffset)));
								}
							};
							...
							onMove = function(event) {
								if(pointer) {
									...
									eventX = event.clientX || event.originalEvent.clientX || event.touches[0].clientX;
									if(dragRange) {
										...
										newLow = percentOffset(newLow);
										newHigh = percentOffset(newHigh);
							...
							onStart = function(event, ptr, rf) {
								...
								var eventX = event.clientX || event.originalEvent.clientX || event.touches[0].clientX;
								if(ref == refSel) {
									dragRange = true;
								...
								newOffset = Math.max(Math.min(newOffset, maxOffset), minOffset);
								var newValue = minValue + (valueRange * percentOffset(newOffset) / 100.0);
								newValue = roundStep(newValue, parseFloat(scope.precision), parseFloat(scope.step), parseFloat(scope.floor),
									parseFloat(scope.ceiling));
								scope[ref] = newValue;
								setPointers();
							...
							setBindings = function() {
								var bindSwipeStart = function(elem, rf, ptr) {
									...
									if(window.navigator.msPointerEnabled) {
										// IE10+
										elem.bind('MSPointerDown', start);
									} else {
										// everything else
										elem.bind('mousedown touchstart', start);
									}
								};
								var bindSwipe = function(elem) {
									...
									if(window.navigator.msPointerEnabled) {
										// IE10+ events
										elem.bind('MSPointerMove', onMove);
										elem.bind('MSPointerUp', onEnd);
									} else {
										// everything else
										elem.bind('mousemove touchmove',onMove);
										elem.bind('mouseup touchend',onEnd);
									}
								};
