describe("Unit: Slider Directive", function() {
	...
	beforeEach(function() {
		...
		element = $compile(
			"<ng-slider floor='{{ slider.floor }}' ceiling='{{ slider.ceiling }}' ng-slider-options='slider.opts'>" +
				"<ng-slider-knob ng-model='knob.value' ng-repeat='knob in slider.knobs'><span style='display: none;'>{{ $viewValue }}</span></ng-slider-knob>" +
			"</ng-slider>"
		)($rootScope);
		element.css('width', 100);
		$document.find('body').append(element).css({margin: 0, padding: 0});
		$rootScope.slider.knobs[0].value = 5;
		$rootScope.$apply();
	});

	it('should create a slider with one knob and two bars', function() {
		expect(element.find('.ng-slider-knob').length).toBe(1);
		expect(element.find('.ng-slider-bar').length).toBe(2);
	});
	...
	it('should place the knob in the middle', function() {
		expect(parseInt(element.find('.ng-slider-knob').css('left'))).toBe(50);
	});

	it('should place the bars according to the knob', function() {
		var bars = element.find('.ng-slider-bar');
	...
	it('should have the correct view value', function() {
		expect(element.find('.ng-slider-knob').text()).toBe('5');
	});
	...
	describe('when the knob is dragged', function() {
		beforeEach(function() {
			var knob = element.find('.ng-slider-knob');
			// click and drag
		...
		it('should move the knob to the correct position', function() {
			expect(parseInt(element.find('.ng-slider-knob').css('left'))).toBe(80);
		});
	...
	describe('when a knob is added', function() {
		...
		it('should add the knob and a bar', function() {
			expect(element.find('.ng-slider-knob').length).toBe(2);
			expect(element.find('.ng-slider-bar').length).toBe(3);
		});
		...
		it('should place the knobs correctly', function() {
			var knobs = element.find('.ng-slider-knob');
		...
		describe('and a knob is dragged', function() {
			beforeEach(function() {
				var knob = element.find('.ng-slider-knob').eq(1);
				// click and drag
			...
			it('should move the knob to the correct position', function() {
				expect(parseInt(element.find('.ng-slider-knob').eq(1).css('left'))).toBe(60);
			});
