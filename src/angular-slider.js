            ...
            function($timeout, $document, $interpolate, $swipe) {
                ...
                return {
                    ...
                    template: 
                        ...
                        "<span class='bar selection'></span><span class='bar unselected low'></span><span class='bar unselected high'></span>" +
                        // the knobs
                        "<span class='pointer low'></span><span class='pointer high'></span>" +
                        // current value bubbles
                        "<span class='bubble low'></span><span class='bubble high'></span><span class='bubble middle'></span><span class='bubble selection'></span>" +
                        // upper and lower limit bubbles
                        "<span class='bubble limit floor'></span><span class='bubble limit ceiling'></span>" +
                        // range sliders used for browsers that support them
                        "<input type='range' class='input low' /><input type='range' class='input high' /><input type='range' class='input selection' />",
                    ...
                    compile : function(element, attributes) {
                        ...
                        var isDualKnob = (attributes.ngModel == null) && ((attributes.ngModelLow != null) && (attributes.ngModelHigh != null)), 
                            ...
                            refLow = isDualKnob ? 'ngModelLow' : 'ngModel', 
                            refHigh = 'ngModelHigh', 
                            refSel = 'selectBar', 
                        ...
                        function setReferences(refs, dual, inputs) {
                            return {
                                ...
                                selBar:         dual ? refs[1] : null,                          // dual knob: the bar between knobs
                                unSelBarLow:    dual ? refs[2] : null,                          // dual knob: the bar to the left of the low knob
                                unSelBarHigh:   dual ? refs[3] : null,                          // dual knob: the bar to the right of the high knob
                                minPtr:         dual ? refs[4] : refs[1],                       // single knob: the knob, dual knob: the low knob
                                maxPtr:         dual ? refs[5] : null,                          // dual knob: the high knob
                                lowBub:         dual ? refs[6] : refs[2],                       // single knob: the value bubble, dual knob: the low value bubble
                                highBub:        dual ? refs[7] : null,                          // dual knob: the high value bubble
                                cmbBub:         dual ? refs[8] : null,                          // dual knob: the range values bubble
                                selBub:         dual ? refs[9] : null,                          // dual knob: the range width bubble
                                flrBub:         dual ? refs[10] : refs[3],                      // the lower limit bubble
                                ceilBub:        dual ? refs[11] : refs[4],                      // the upper limit bubble
                                minInput:       inputs ? (dual ? refs[12] : refs[5]) : null,    // single knob: the actual slider input, dual knob: the low value slider input
                                maxInput:       inputs ? (dual ? refs[13] : null) : null,       // dual knob: the high value slider input
                                selInput:       inputs ? (dual ? refs[14] : null) : null        // dual knob: the selection slider input
                            };
                        }
                        ...
                        bindHtml(refs.selBub, "Range: " + expression("translation(" + refHigh + " - " + refLow + ")"));
                        bindHtml(refs.lowBub, expression("translation(" + refLow + ")"));
                        bindHtml(refs.highBub, expression("translation(" + refHigh + ")"));
                        bindHtml(refs.cmbBub, expression("translation(" + refLow + ")") + " - " + expression("translation(" + refHigh + ")"));
                        
                        // start to compile watchables
                        watchables = [refLow, 'floor', 'ceiling', 'precision', 'stickiness'];
                        if(isDualKnob) {
                        ...
                        return {
                            post: function(scope, element, attributes) {
                                ...
                                function dimensions() {
                                    ...
                                    if(angular.isUndefined(scope.precision) || !scope.precision) {
                                        scope.precision = 0;
                                    } else {
                                    ...
                                    if(angular.isUndefined(scope.step) || !scope.step || scope.step == 0) {
                                    ...
                                    if(angular.isUndefined(scope.stickiness)) {
                                        scope.stickiness = KNOB_STICKINESS;
                                    } else {
                                        scope.stickiness = parseFloat(scope.stickiness);
                                    }
                                    ...
                                    angular.forEach(watchables, function(watchable) {
                                        ...
                                        if(watchable == refLow || watchable == refHigh) {
                                            ...
                                            scope[watchable] = roundStep(parseFloat(scope[watchable]), scope.precision, scope.step, parseFloat(scope.floor), parseFloat(scope.ceiling));
                                        } else if(watchable == 'buffer') {
                                            // this is the buffer so make sure it aligns with the steps
                                            scope[watchable] = stepBuffer(scope.step, parseFloat(scope[watchable]))
                                        }
                                    });
                                    ...
                                    if(isDualKnob) {
                                        ...
                                        if(scope.buffer > 0 && diff < scope.buffer) {
                                            ...
                                            // so find the middle
                                            var avg = (scope[refLow] + scope[refHigh]) / 2;
                                ...
                                function updateDOM() {
                                    ...
                                    function fitToBar(element) {
                                        return offset(
                                            ...
                                            offsetFromPercent(
                                                percentFromOffset(
                                                    bringOffsetInRange(offsetLeft(element))
                                                )
                                            )
                                        );
                                    }
                                    ...
                                    function setPointers() {
                                         ...
                                         * @type {number}
                                         */
                                        var stepWidthPercent = percentFromValue(scope.step + minValue);

                                        /**
                                         * The width in percent of the buffer
                                         * @type {number}
                                         */
                                        var bufferWidthPercent = percentFromValue(scope.buffer + minValue);
                                         ...
                                         * @type {number}
                                         */
                                        var rawLowPercent = percentFromValue(scope[refLow]);
                                        ...
                                        if(isDualKnob) {
                                             ...
                                             * @type {number}
                                             */
                                            var rawHighPercent = percentFromValue(scope[refHigh]);
                                            ...
                                            offset(
                                                refs.selBub,
                                                offsetFromPercent(
                                                    stretchedLowPercent + percentFromOffset(halfWidth(refs.selBar) - halfWidth(refs.selBub) + minOffset)
                                                )
                                            );
                                            offset(
                                                refs.cmbBub,
                                                offsetFromPercent(
                                                    stretchedLowPercent + percentFromOffset(halfWidth(refs.selBar) - halfWidth(refs.cmbBub) + minOffset)
                                                )
                                            );
                                        ...
                                        if(pointer) {
                                            ...
                                            scope.$apply(function() {
                                                ...
                                                if(dragRange) {
                                                    ...
                                                    newLow = roundStep(newLow, scope.precision, scope.step, scope.floor, scope.ceiling);
                                                    newHigh = roundStep(newHigh, scope.precision, scope.step, scope.floor, scope.ceiling);
                                    ...
                                    function onStart(event, ptr, rf) {
                                        ...
                                        var newValue = minValue + (valueRange * percentFromOffset(bringOffsetInRange(currentX + minOffset - offsetLeft(element))) / 100.0);
                                        ...
                                        if(ref == refSel) {
                                            ...
                                            lowOffset = currentX - offsetFromValue(scope[refLow]) - halfWidth(refs.minPtr);
                                            highOffset = offsetFromValue(scope[refHigh]) - currentX + halfWidth(refs.maxPtr);
                                        }
                                        
                                        // round the value to a step
                                        newValue = roundStep(newValue, scope.precision, scope.step, scope.floor, scope.ceiling);
                                ...
                                for(var _j = 0, _len1 = watchables.length; _j < _len1; _j++) {
                                    scope.$watch(watchables[_j], function() {
                                        updateDOM();
                                    });
                                }
