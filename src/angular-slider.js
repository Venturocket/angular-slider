    ...
    ['$timeout', '$document', '$interpolate', '$swipe', function($timeout, $document, $interpolate, $swipe) {
        ...
        return {
            ...
            compile : function(element, attributes) {
                var ceilBub, cmbBub, e, flrBub, fullBar, highBub, lowBub, maxPtr, minPtr, isDualKnob, rangeInputsSupported, refHigh, refLow, refSel, selBar, unSelBarLow, unSelBarHigh, selBub, minInput, maxInput, selInput, watchables, _i, _len, _ref, _ref1;

                if(attributes.translate) {
                    attributes.$set('translate', "" + attributes.translate + "(value)");
                }
                isDualKnob = (attributes.ngModel == null) && ((attributes.ngModelLow != null) && (attributes.ngModelHigh != null));
                rangeInputsSupported = AngularSlider.rangeInputs || false;
                _ref = (function() {
                    ...
                    = _ref[6], highBub = _ref[7], cmbBub = _ref[8], selBub = _ref[9], flrBub = _ref[10], ceilBub = _ref[11], minInput
                    = _ref[12], maxInput = _ref[13], selInput = _ref[14];
                ...
                return {
                    post: function(scope, element, attributes) {
                        ...
                        dimensions = function() {
                            ...
                            if(isDualKnob && parseFloat(scope[refHigh]) < parseFloat(scope[refLow])) {
                        ...
                        updateDOM = function() {
                            ...
                            fitToBar = function(element) {
                                return offset(element,
                                    percentToOffset(percentFromOffset(Math.min(Math.max(minOffset, offsetLeft(element)), maxOffset))));
                            };
                            percentStretch = function(percent, maxPercent, end) {
                                ...
                                if(end) {
                                    return (
                                               Math.sin((Math.min(Math.abs(percent / maxPercent), 1) * Math.PI) - (Math.PI / 2)) + 1
                                               ) * sign * maxPercent / 6;
                                }
                            ...
                            setPointers = function() {
                                ...
                                if(isDualKnob) {
                                    ...
                                    offset(selBub,
                                        percentToOffset(newLowPercent + percentFromOffset(halfWidth(selBar) - halfWidth(selBub) + minOffset)));
                                    offset(cmbBub,
                                        percentToOffset(newLowPercent + percentFromOffset(halfWidth(selBar) - halfWidth(cmbBub) + minOffset)));
                                    if(rangeInputsSupported) {
                                        minInput.css({
                                ...
                                if(rangeInputsSupported) {
                                    var bindSlider = function(elem, rf, ptr) {
                                        ...
                                        if(window.navigator.pointerEnabled) {
                                            elem.on('pointerdown', start).on('pointermove', onMove).on('pointerup', end).on('pointercancel', onEnd);
                                        } else if(window.navigator.msPointerEnabled) {
                                            elem.on('MSPointerDown', start).on('MSPointerMove', onMove).on('MSPointerUp', end).on('MSPointerCancel',
                                                onEnd);
                                        } else {
                                            $swipe.bind(elem, {
                                                start : start,
